...
class QuartzEnv5(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(
            np.array([-1.0, -math.inf, 0.0, 0.0], dtype=np.float32),  # type: ignore
        ...
        self._prev_position = WorkzoneRelativeCoordinates(-1.0, 1.0)
        self._params = {}
    ...
    def _sim_step(self, act: bool) -> Tuple[float, bool]:
        ...
        if act:
            dest_wz = WorkzoneRelativeCoordinates(
                self._current_position[0] * -1.0, self._current_position[1])
        else:
            dest_wz = WorkzoneRelativeCoordinates(
